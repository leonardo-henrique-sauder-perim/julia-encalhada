...
app.post("/turma", (req, res) => {
    ...
    if (
        !nome_turma ||!codigo ||curso === undefined ||!ano ||!coordenador ||!ano_letivo
    ) {
        return res.status(400).json({
            error: "nome, codigo, curso, ano , ano_letivo e coordenador são obrigatórios.",
        });
    }
...
// GET: Listar turma (filtro listar por codigo ou ano )
...
app.put("/turma/:codigo", (req, res) => {
    ...
    const query = `UPDATE turma SET nome_turma =?, codigo=?, turno=?, curso=?, ano=?, ano_letivo=?, capacidade=?, sala=?, coordenador=?, observacao=? `;
    db.run(
        query,
        [
            nome_turma,
