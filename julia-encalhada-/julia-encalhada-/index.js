...
app.post("/turma", (req, res) => {
    ...
    // CORREÇÃO: Verificação correta dos campos obrigatórios
    if (!nome_turma || !codigo || !curso || !ano || !coordenador || !ano_letivo) {
        return res.status(400).json({
            error: "nome_turma, codigo, curso, ano, ano_letivo e coordenador são obrigatórios.",
        });
    }

    const query = `INSERT INTO turma (nome_turma, codigo, turno, curso, ano, ano_letivo, capacidade, sala, coordenador, observacao) VALUES (?,?,?,?,?,?,?,?,?,?)`;
    db.run(
        query,
        [
            nome_turma,
...
// GET: Listar turma (filtro por codigo ou ano)
app.get("/turma", (req, res) => {
    const codigo = req.query.codigo || "";
    const ano = req.query.ano || "";

    if (codigo) {
        // CORREÇÃO: Query corrigida para buscar na tabela turma
        ...
        // CORREÇÃO: Query corrigida para buscar na tabela turma
        const query = `SELECT * FROM turma WHERE ano LIKE ?`;
        db.all(query, [`%${ano}%`], (err, rows) => {
            if (err) {
                console.error("Erro no SELECT turma:", err.message);
                return res.status(500).json({ error: "Erro ao buscar turma." });
            }
...
// PUT: Atualizar turma por codigo
app.put("/turma/:codigo", (req, res) => {
    ...
    // CORREÇÃO: Query completa com WHERE clause
    const query = `UPDATE turma SET nome_turma =?, turno=?, curso=?, ano=?, ano_letivo=?, capacidade=?, sala=?, coordenador=?, observacao=? WHERE codigo = ?`;
    db.run(
        query,
        [
            nome_turma,
        ...
        function (err) {
            if (err) {
                console.error("Erro no UPDATE turma:", err.message);
                return res
                    .status(500)
